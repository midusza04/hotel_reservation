...
3. Wydzielic audit log - zrobione.

### Etap C - rozproszenie i niezawodnosc

1. Uruchomic klaster Ray (head + workers) - zrobione.
2. Dodac limity zasobow aktorow (`num_cpus`, placement) - zrobione.
3. Dodac retry/backoff i timeouty miedzy aktorami - zrobione.
4. Dodac monitoring (Prometheus/Grafana + log aggregation) - zrobione.

### Etap D - frontend

1. Zmienic placeholder na SPA (np. React/Vite) - zrobione.
2. Widoki: logowanie, lista hoteli, szczegoly, checkout, historia - zrobione.
3. Integracja z backend API + obsluga bledow i loading states - zrobione.
...
Etapy A, B, C, D sa w pelni zrealizowane. Mozna teraz przejsc do:

**D - Frontend (SPA) - ZROBIONE:**
- Vanilla JS SPA z hash-routingiem (bez build step).
- Widoki: logowanie, wyszukiwanie hoteli, checkout, historia rezerwacji, panel admina.
